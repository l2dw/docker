...
```
# Traefik / multi-app uniqueness (Dokploy may inject the same key)
APP_NAME=myapp
...
- **`APP_NAME`:** Traefik router/service (and app-owned middleware) names use `${APP_NAME:-<projet>}`. Compose service YAML key stays `<projet>` for DNS; `homepage.siteMonitor` uses that DNS name. **Do** list `APP_NAME=<projet>` in root and project `.env.example` (Dokploy may override). Do not nest `${APP_NAME}` inside other `.env` values — keep volume/config name keys as literals and document renaming them when `APP_NAME` changes.
