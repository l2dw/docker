...
description: Scaffolds a new application Docker stack in this docker-templates repo (dedicated git branch, project directory, Makefile, docker-compose.yml with optional Traefik/Homepage labels or compose.yml symlink when unlabeled, base_path check, root symlinks). Use when creating a new app stack, docker-compose.yml, Swarm stack, wrapping an image, or when the user asks to dockeriser une application, créer un stack, copier le template tpl, or follow the template/dictfp/dokploy structure.
---
...
Make targets use `docker-compose.yml`. Use `compose.yml` when Traefik/Homepage must not see the service (unlabeled copy, or the symlink when unlabeled).
