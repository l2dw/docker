...
Network: `name: ${DEFAULT_NETWORK_NAME:-<projet>-network}` and `external: ${DEFAULT_NETWORK_EXTERNAL:-false}` (unquoted). **Default is a stack-local overlay** (e.g. `immich-network`, `myapp-network`) with `external=false` so Swarm/Compose can create it. Pairing (two keys; **no nested** `${A:-${B}}`): **an external shared overlay name (e.g. `dokploy-network`) ⇒ `DEFAULT_NETWORK_EXTERNAL=true`**; any stack-local name (by default `<projet>-network`) ⇒ `false`. `<projet>-setup` must upsert `DEFAULT_NETWORK_EXTERNAL` to match `DEFAULT_NETWORK_NAME` while keeping them **in the stack's `.env`/`.env.example`** (never a global in root `.env.example`) — root `.env.example` must **not** define `DEFAULT_NETWORK_*` (a stack that isn't that overlay cannot carry its name). When a shared overlay + Traefik is wanted, set `DEFAULT_NETWORK_NAME=<shared-overlay>` and `DEFAULT_NETWORK_EXTERNAL=true` **in the stack's `.env`**. No `x-*` keys. No quotes around `${…}` booleans (`privileged`, `external`). Do **not** add network `aliases` unless the user asks or a Dokploy-style stable hostname is required. Do **not** inject `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` into the service unless the user asks. Details in [conventions.md](conventions.md).
...
- Include `DEFAULT_NETWORK_NAME=<projet>-network`, `DEFAULT_NETWORK_EXTERNAL=false` (stack-local default). If the user wants the shared overlay: `DEFAULT_NETWORK_NAME=<shared-overlay>` and `DEFAULT_NETWORK_EXTERNAL=true` — **in the stack's `.env.example` only** (never root). Root `.env.example` must **not** define `DEFAULT_NETWORK_*`. Pairing: an external shared overlay name ⇒ `EXTERNAL=true`; a stack-local name (default `<projet>-network`) ⇒ `false`.
...
| Target | Role |
|--------|------|
| `<projet>-setup` | Ensure `<projet>/.env` (from `.env.example`), generate missing secrets, warn on placeholder domains / incomplete OAuth, **sync `DEFAULT_NETWORK_EXTERNAL`** (an external shared overlay name ⇒ `true`; a stack-local name — default `<projet>-network`, including empty → fallback `<projet>-network` — ⇒ `false`). It may source an existing root `.env` value for the network only to keep it consistent, but **must not** write `DEFAULT_NETWORK_*` to root `.env.example` (stack-owned). **Name is `<projet>-setup`, not `<projet>-stack-setup`.** |
