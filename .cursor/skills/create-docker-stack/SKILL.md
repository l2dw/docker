...
Do not reuse `tpl` / `TPL` in generated files. **Do** put `APP_NAME=<projet>` in `.env.example` (root + project) so Traefik router names and local Compose/Swarm stay unique without Dokploy; Dokploy may still inject/override `APP_NAME`. Compose labels keep `${APP_NAME:-<projet>}`. Do **not** nest `${APP_NAME}` inside other `.env` values (Compose does not expand them — use literals and rename volume/config names when `APP_NAME` changes).
...
- **Do** add `APP_NAME=<projet>` near the top of root and project `.env.example` (same value). Traefik labels still use `${APP_NAME:-<projet>}` in compose. Dokploy may inject/override; the `.env.example` default covers Make/Compose/Swarm without Dokploy. Companion names (`*_VOLUME_NAME`, Swarm config names, etc.) stay **literals** aligned with `<projet>` — do not write `FOO=${APP_NAME}_bar` in `.env` files.
...
When Traefik labels are used, note that `APP_NAME` scopes router/service names (`${APP_NAME:-<projet>}` in compose; default listed in `.env.example`). If the operator changes `APP_NAME`, they must also rename related literal volume/config env keys to avoid clashes.
